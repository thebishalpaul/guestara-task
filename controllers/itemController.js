...
exports.getItemsByCategory = async (req, res) => {
  try {
    // Find the category by its ID using the Category model and populate the 'subcategories' field
    ...
    // Find all items that belong to the category using the Item model
    const allItems = await Item.find({ category: categoryId });

    // If no items are found under the category, return an empty array
...
exports.getItemByName = async (req, res) => {
  try {
    ...
    const item = await Item.findOne({ slug: name.toLowerCase() });
